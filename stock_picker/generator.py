...
def render_card(stock: dict, eval_result: dict) -> str:
    ...
    inst_dir   = "買超" if s["inst_5d_total"] > 0 else "賣超"
    inst_color = "#22c55e" if s["inst_5d_total"] > 0 else "#ef4444"
...
<div style="background:#0f172a;border:1px solid #1e293b;border-radius:16px;padding:20px;
  ...
  <div style="padding-right:60px">
    ...
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      ...
      <span style="font-size:10px;padding:2px 8px;border-radius:20px;
            background:#1e293b;color:{inst_color}">法人連{inst_dir} {s['consecutive_buy_days']}日</span>
      {kw_badges}
    </div>
  </div>
  ...
  <div style="display:flex;gap:16px;font-size:12px;color:#64748b;flex-wrap:wrap">
    ...
    <span>近20日 <strong style="color:{inst_color}">{s['inst_20d_total']:+,}</strong> 張</span>
  </div>
...
def render_page(stocks_with_eval: list[tuple[dict, dict]], generated_at: str) -> str:
    """組合完整 HTML 頁面"""
    # 依分數排序
    sorted_items = sorted(stocks_with_eval, key=lambda x: x[1].get("score", 0), reverse=True)

    cards_html = "\n".join(render_card(s, e) for s, e in sorted_items)

    count = len(sorted_items)
    high_count  = sum(1 for _, e in sorted_items if e.get("score", 0) >= 70)
...
<title>低基期選股名單 — {generated_at[:10]}</title>
<style>
...
<div class="container">
  ...
  <div class="header">
    <h1>🔍 低基期選股名單</h1>
    <p>產生時間：{generated_at}　｜　篩選條件：有題材新聞 ＋ 法人連買 ≥3日 ＋ 量能放大 ≥1.2x　｜　由 Claude AI 評分</p>
  </div>

  <div class="stats">
    <div class="stat"><strong>{count}</strong>檔通過篩選</div>
