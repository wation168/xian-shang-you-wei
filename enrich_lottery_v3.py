...
"""
import os, re, json, html as html_mod, sys, shutil, datetime
import i18n_data

DRY_RUN = True  # 預設安全模式：只列出會改什麼，不真的寫檔。用 --apply 才會真的執行
...
def gen_faq(slug, lang):
    ...
    elif lang in i18n_data.FAQ_TEMPLATES:
        t = i18n_data.FAQ_TEMPLATES[lang]
        tc = i18n_data.TAX_CLAIM_I18N.get(slug, {}).get(lang, {"tax": c["tax"], "claim": c["claim"]})
        bt = t["bonus_text"].format(bonus=c['bonus'], brange=c['brange']) if c['bonus']>0 else ""
        fmt = dict(n=n, range=c['range'], pick=c['pick'], bonus_text=bt, bet=c['bet'], days=c['days'], time=c['time'], odds=c['odds'])
        return [
            (t["q1"].format(**fmt), t["a1"].format(**fmt)),
            (t["q2"].format(**fmt), t["a2"].format(**fmt)),
            (t["q3"].format(**fmt), tc["tax"]),
            (t["q4"].format(**fmt), tc["claim"]),
            (t["q5"].format(**fmt), t["a5"].format(**fmt)),
        ]
    else:
        # en 預設（也是找不到對應語言時的保底）
        return [
...
def build_faq_block(slug, lang):
    ...
    title = {"zh-TW":"\u5e38\u898b\u554f\u984c","zh-CN":"\u5e38\u89c1\u95ee\u9898"}.get(lang)
    if title is None:
        title = i18n_data.FAQ_TEMPLATES.get(lang, {}).get("faq_title", "FAQ")
...
def build_howto_block(slug, lang):
    ...
    if lang.startswith("zh"):
        ...
        tc = {"tax": c["tax"], "claim": c["claim"]}
        l_tax, l_claim = "\u7a05\u7387", "\u9818\u734e"
    elif lang in i18n_data.HOWTO_TEMPLATES:
        t = i18n_data.HOWTO_TEMPLATES[lang]
        tc = i18n_data.TAX_CLAIM_I18N.get(slug, {}).get(lang, {"tax": c["tax"], "claim": c["claim"]})
        bt = t["bonus_text"].format(bonus=c['bonus'], brange=c['brange']) if c['bonus']>0 else ""
        fmt = dict(n=n, range=c['range'], pick=c['pick'], bonus_text=bt, bet=c['bet'], days=c['days'], time=c['time'], odds=c['odds'])
        steps = [t["step1"].format(**fmt), t["step2"].format(**fmt), t["step3"].format(**fmt), t["step4"].format(**fmt)]
        t_how, t_tax = t["t_how"].format(**fmt), t["t_tax"]
        l_tax, l_claim = t["l_tax"], t["l_claim"]
    else:
  ...
  <div class="card" style="margin-top:20px">
    ...
    <p><strong>{l_tax}:</strong> {tc["tax"]}</p>
    <p><strong>{l_claim}:</strong> {tc["claim"]}</p>
  </div>'''


def _generic_faq_block(slug, lang, data, title_key="title"):
    """共用：把 HISTORY_FAQ / STATS_FAQ 這種 {lang: {title, q:[(q,a),...]}} 結構轉成 FAQ HTML+Schema"""
    c = LOTTERIES[slug]
    n = c.get(f"name_{lang[:2]}", c["name"])
    entry = data.get(lang) or data.get("en")
    title = entry[title_key]
    items = ""
    schema_items = []
    for qt, at in entry["q"]:
        q = qt.format(n=n, range=c['range'])
        a = at.format(n=n, range=c['range'])
        eq, ea = html_mod.escape(q), html_mod.escape(a)
        items += f'<div class="faq-item"><div class="faq-q">{eq}</div><div class="faq-a">{ea}</div></div>\n'
        schema_items.append({"@type":"Question","name":q,"acceptedAnswer":{"@type":"Answer","text":a}})
    schema = json.dumps({"@context":"https://schema.org","@type":"FAQPage","mainEntity":schema_items}, ensure_ascii=False)
...
def build_history_faq_block(slug, lang):
    return _generic_faq_block(slug, lang, i18n_data.HISTORY_FAQ)


def build_statistics_faq_block(slug, lang):
    return _generic_faq_block(slug, lang, i18n_data.STATS_FAQ)


# ============================================================
# NUMBER-GENERATOR.HTML 加厚（修 meta description 空字串 bug + 加介紹段落 + FAQ）
# ============================================================
def fix_number_generator_page(filepath, lang):
    with open(filepath, "r", encoding="utf-8") as f:
        html = f.read()
    content = i18n_data.NG_CONTENT.get(lang, i18n_data.NG_CONTENT["en"])
    modified = False

    # 1. 修 meta description 空字串 bug
    empty_desc_pattern = re.compile(r'<meta name="description" content="">')
    if empty_desc_pattern.search(html):
        esc = html_mod.escape(content["meta_desc"], quote=True)
        html = empty_desc_pattern.sub(f'<meta name="description" content="{esc}">', html)
        modified = True

    # 2. 加介紹段落（放在 H1 卡片後面）+ FAQ（放在 lottery-grid 後面）
    has_intro_p = 'ng-intro-block' in html
    if not has_intro_p:
        intro_html = f'<div class="card ng-intro-block" style="margin-top:16px"><p style="font-size:14px;color:#4A5568;line-height:1.8">{html_mod.escape(content["intro"])}</p></div>'
        if '<div class="ad-slot" id="ad1"></div>' in html:
            html = html.replace('<div class="ad-slot" id="ad1"></div>', intro_html + '\n  <div class="ad-slot" id="ad1"></div>', 1)
            modified = True

        # FAQ block, 放在 </div>\n</div>\n<div class="disclaimer"> 之前（頁尾disclaimer前）
        items = ""
        schema_items = []
        for q, a in content["faq"]:
            ...
            schema_items.append({"@type":"Question","name":q,"acceptedAnswer":{"@type":"Answer","text":a}})
        schema = json.dumps({"@context":"https://schema.org","@type":"FAQPage","mainEntity":schema_items}, ensure_ascii=False)
        faq_html = f'''<div class="card" style="margin-top:20px">
    <h2>\u2753 {content["faq_title"]}</h2>
    {items}
  </div>
  <script type="application/ld+json">{schema}</script>'''
        if '<div class="disclaimer">' in html:
            html = html.replace('<div class="disclaimer">', faq_html + '\n<div class="disclaimer">', 1)
            modified = True
        elif '</body>' in html:
...
# ============================================================
# INDEX.HTML 加厚（首頁加一小段介紹文字，純目錄頁不加FAQ）
# ============================================================
def fix_index_intro(filepath, lang):
    with open(filepath, "r", encoding="utf-8") as f:
        html = f.read()
    intro_text = i18n_data.INDEX_INTRO.get(lang, i18n_data.INDEX_INTRO["en"])
    modified = False

    has_intro_p = 'idx-intro-block' in html
    if not has_intro_p:
        intro_html = f'<div class="card idx-intro-block"><p style="font-size:14px;color:#4A5568;line-height:1.8">{html_mod.escape(intro_text)}</p></div>'
        if '<div class="ad-slot" id="ad1"></div>' in html:
            html = html.replace('<div class="ad-slot" id="ad1"></div>', intro_html + '\n  <div class="ad-slot" id="ad1"></div>', 1)
            modified = True
...
_ENGLISH_FAQ_MARKERS = ["How do I play", "What are the odds of winning", "How do I claim a", "Where can I check", "winnings taxed?"]

def _looks_like_english_faq(html):
    """偵測頁面裡的FAQ問答是不是舊版bug產生的英文內容（用於非en/zh語言頁）"""
    return any(m in html for m in _ENGLISH_FAQ_MARKERS)


_HOWTO_BLOCK_RE = re.compile(
    r'<div class="card" style="margin-top:20px">\s*<h2>\U0001f4d6.*?</div>\s*'
    r'<div class="card" style="margin-top:20px">\s*<h2>\U0001f4b0.*?</div>\s*',
    re.DOTALL)
_FAQ_BLOCK_RE = re.compile(
    r'<div class="card" style="margin-top:20px">\s*<h2>\u2753.*?</script>\s*',
    re.DOTALL)

def _strip_old_howto_faq_block(html):
    """移除舊版（語言錯誤）注入的 howto(📖💰) 區塊 + FAQ(❓) 區塊，準備重新注入正確語言版本"""
    html = _HOWTO_BLOCK_RE.sub('', html)
    html = _FAQ_BLOCK_RE.sub('', html)
    return html
...
def enrich_file(filepath):
    ...
    # NEW: 偵測「有FAQ但語言是錯的」— 舊版bug只會在這7種語言出現，且FAQ內容其實是英文
    wrong_lang_faq = has_real_faq and lang in i18n_data.FAQ_TEMPLATES and _looks_like_english_faq(html)

    if ptype == "intro" and (not has_real_faq or wrong_lang_faq):
        if wrong_lang_faq:
            html = _strip_old_howto_faq_block(html)
    ...
    if ptype == "results" and (not has_real_faq or wrong_lang_faq):
        if wrong_lang_faq:
            html = _strip_old_howto_faq_block(html)
        faq = build_faq_block(slug, lang)
        if '<div class="lang-bar">' in html:
    ...
    # 3b. NEW: Inject FAQ for history pages (原本完全沒涵蓋)
    if ptype == "history" and not has_real_faq:
        faq = build_history_faq_block(slug, lang)
        if '<div class="lang-bar">' in html:
            html = html.replace('<div class="lang-bar">', faq + '\n  <div class="lang-bar">')
            modified = True

    # 3c. NEW: Inject FAQ for statistics pages (原本完全沒涵蓋)
    if ptype == "statistics" and not has_real_faq:
        faq = build_statistics_faq_block(slug, lang)
        if '<div class="lang-bar">' in html:
            html = html.replace('<div class="lang-bar">', faq + '\n  <div class="lang-bar">')
            modified = True
...
def backup_base():
    ...
    backup_dir = BASE.rstrip("/\\") + f"_backup_{ts}"
    print(f"\U0001f4e6 \u5099\u4efd {BASE} \u5230 {backup_dir} ...")
...
def main():
    ...
    print("=" * 60)
    print("Enrich Lottery Pages v3 (\u8a9e\u8a00\u4fee\u6b63 + history/statistics + number-generator/index \u52a0\u539a)")
    print(f"\u6a21\u5f0f: {'DRY-RUN\uff08\u53ea\u5217\u51fa\u4e0d\u5beb\u6a94\uff09' if DRY_RUN else '\u6b63\u5f0f\u57f7\u884c'}")
    print("=" * 60)
    ...
    # Step 2: Enrich lottery pages (intro/results/history/statistics)
    ...
    print(f"\n--- Summary: lottery pages ---")
    print(f"Lottery pages: {total} total, {enriched} enriched")

    # Step 3: Fix number-generator.html + index.html hub pages (10 languages)
    print("\n--- Enriching hub pages (number-generator.html / index.html) ---")
    hub_fixed = 0
    for lang, folder in LANG_FOLDERS.items():
        lang_dir = os.path.join(BASE, folder) if folder else BASE
        ng_path = os.path.join(lang_dir, "number-generator.html")
        idx_path = os.path.join(lang_dir, "index.html")
        if os.path.isfile(ng_path):
            if fix_number_generator_page(ng_path, lang):
                hub_fixed += 1
                print(f"  \u2705 {lang}/number-generator.html")
        if os.path.isfile(idx_path):
            if fix_index_intro(idx_path, lang):
                hub_fixed += 1
                print(f"  \u2705 {lang}/index.html")

    print(f"\n--- Summary ---")
    print(f"Lottery pages: {total} total, {enriched} enriched")
    print(f"Hub pages enriched: {hub_fixed}")
    print("Done!" if not DRY_RUN else "Dry-run \u5b8c\u6210\uff0c\u6c92\u6709\u5beb\u5165\u4efb\u4f55\u6a94\u6848\u3002\u78ba\u8a8d\u4e0a\u9762\u6e05\u55ae\u6c92\u554f\u984c\u5f8c\uff0c\u52a0 --apply \u53c3\u6578\u91cd\u65b0\u57f7\u884c\u624d\u6703\u771f\u7684\u5beb\u6a94\u6848\u3002")
